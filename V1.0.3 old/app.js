...
function getWordData(fromIndex, dir) {
  ...
  while (r >= 0 && r < ROWS && c >= 0 && c < COLS) {
    ...
    // La case garde sa lettre normalement
    const char = cells[idx].letter ? cells[idx].letter.toUpperCase() : "_";
    word += char;
    indexes.push(idx);

    // Si la case a un séparateur, on ajoute un tiret dans le mot associé (sans consommer de case)
    if (cells[idx].isSeparator) {
      word += "-";
    }
...
function getDoubleHalfWordData(defIndex, half, dir) {
  ...
  if (cells[anchorIdx].isSeparator) {
    word += "-";
  }
...
function prioritizeDirectionFromDefinitionArrow(defIndex, half, dir) {
  ...
  if (targetIndex === null) {
    targetIndex = wordIndexes.find(idx => cells[idx] && cells[idx].type === "letter");
    if (targetIndex === undefined) targetIndex = null;
  }
  ...
  markAsDirty();
  render();
  selectCellAndFocus(targetIndex);
}
...
// Renvoie les indices des cases du mot non prioritaire en cas d'intersection
function getNonPriorityCells() {
...
function updateGridArrowHighlights() {
  const grid = document.getElementById("grid");
  if (!grid) return;

  // Réinitialise l'état actif sur toutes les flèches de la grille
  ...
  if (selected === null || selected === undefined || !cells[selected]) return;
  ...
  if (cell.type === "letter") {
    ...
    if (parents.length > 0) {
      const wantedDir = cell.priorityDir || currentInputDir;
      const activeParent = parents.find(p => p.dir === wantedDir) || parents.find(p => p.dir === currentInputDir) || parents[0];
    ...
    targetDefIndex = selected;
    targetHalf = null;
  } else if (cell.type === "double") {
    targetDefIndex = selected;
    targetHalf = currentDoubleHalf === "bottom" ? "bottom" : "top";
  }
...
function updateHighlights() {
  ...
  const nonPriorityIndexes = getNonPriorityCells();
...
function render() {
  ...
  cells.forEach((cell, index) => {
    ...
    const isHighlighted = highlightedIndexes.includes(index) || hoveredWordIndexes.includes(index);
...
function updatePanel() {
  ...
  if (selected === null) {
    ...
    // Masquer la section mot mystère si aucune case n'est sélectionnée
  ...
  // 1. Case définition : uniquement valeur et sens d'écriture (pas de box de mot formé)
  if (cell.type === "definition") {
  ...
  // 2. Case double définition : uniquement définitions haut/bas et leurs sens d'écriture
  if (cell.type === "double") {
    document.getElementById("mysterySection").style.display =  "none"
    const topDir = (cell.top && cell.top.arrow) || "E";
    const botDir = (cell.bottom && cell.bottom.arrow) || "S";
  ...
  // 3. Case lettre : affichage des mots associés
  if (cell.type === "letter") {
  ...
  // Gestion des options de séparateur (uniquement pour les cases lettres)
  ...
  if (dir) {
    ...
    if (priorityLetterIdx !== undefined && priorityLetterIdx !== null) {
      ...
      iconSpan.addEventListener("click", (e) => {
        ...
        markAsDirty();
        render();
        selectCellAndFocus(priorityLetterIdx);
      });
    }
...
function toggleCellSeparator(isChecked) {
  ...
  cells[selected].isSeparator = isChecked;
  if (!isChecked) {
    cells[selected].borderRight = false;
    cells[selected].borderBottom = false;
  }
  updatePanel();
  render();
  markAsDirty();
...
function toggleCellBorderRight(isChecked) {
  if (selected === null || cells[selected].type !== "letter") return;
  cells[selected].borderRight = isChecked;
  render();
  markAsDirty();
}

function toggleCellBorderBottom(isChecked) {
  if (selected === null || cells[selected].type !== "letter") return;
  cells[selected].borderBottom = isChecked;
  render();
  markAsDirty();
}